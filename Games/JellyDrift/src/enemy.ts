...
interface Enemy {
  ...
  wanderCd: number;
  contactCd: number;
  lastZapId: number; // last discharge that damaged this enemy
  hurt: number; // hurt-flash timer
  dead: boolean;
  deathT: number;
}

function spawnEnemy(nearCam: boolean): Enemy {
...
export const enemySystem = {
  ...
  init(): void {
    this.enemies = [];
    for (let i = 0; i < COUNT; i++) this.enemies.push(spawnEnemy(false));
  },
  ...
  update(dt: number): void {
    ...
    for (let i = this.enemies.length; i < before; i++) this.enemies.push(spawnEnemy(true));
  },
...
function clampSpeed(e: Enemy, max: number): void {
...
function drawEnemy(sx: number, sy: number, e: Enemy, time: number): void {
  const s = e.size;
  const dead = e.dead;
