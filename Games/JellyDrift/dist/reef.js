...
function drawCoral(x, baseY, c) {
    const fill = `hsl(${c.hue}, ${c.sat}%, ${c.light}%)`;
    if (c.kind === "branch")
        drawBranchCoral(x, baseY, c, fill);
    else if (c.kind === "fan")
        drawFanCoral(x, baseY, c, fill);
    else
        drawTubeCoral(x, baseY, c, fill);
}
/** Recursive branching coral drawn as tapering strokes. */
function drawBranchCoral(x, baseY, c, fill) {
    const rand = mulberry32(c.seed);
    ctx.strokeStyle = fill;
    ctx.lineCap = "round";
    const branch = (bx, by, angle, len, width) => {
        if (len < 8 || width < 1)
            return;
...
function drawTubeCoral(x, baseY, c, fill) {
    const rand = mulberry32(c.seed);
